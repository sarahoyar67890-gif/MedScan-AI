...
BEST_MODEL_PATH = CHECKPOINT_DIR / "medscan_resnet18_best.pt"   # kept for backward compatibility
LAST_MODEL_PATH = CHECKPOINT_DIR / "medscan_resnet18_last.pt"   # (predictor.py still reads this pair)
...
# ---------------------------------------------------------------------------
IMAGE_SIZE = 224  # standard input resolution for all three architectures below
IMAGENET_MEAN = [0.485, 0.456, 0.406]
IMAGENET_STD = [0.229, 0.224, 0.225]

# The final conv block used for Grad-CAM on torchvision's resnet18.
# (Each architecture also carries its own GRADCAM_TARGET_LAYER in its model
# module — see models/registry.py — this constant stays only because
# explainability/gradcam.py currently reads it directly for the resnet18 path.)
GRADCAM_TARGET_LAYER = "layer4"

# ---------------------------------------------------------------------------
# Model registry — architectures available to train/evaluate/serve.
#
# "resnet18" is the original, already-trained-toward binary screening
# architecture. "efficientnet_b0" and "convnext_tiny" are new additions —
# see models/registry.py for the shared interface all three implement.
# DEFAULT_ARCHITECTURE keeps every existing script (predictor.py, the
# Streamlit app) working unmodified: they never reference this list, they
# just keep using resnet18 via config.BEST_MODEL_PATH like before.
# ---------------------------------------------------------------------------
AVAILABLE_ARCHITECTURES = ["resnet18", "efficientnet_b0", "convnext_tiny"]
...
FINETUNE_EPOCHS = 10
FINETUNE_LR = 1e-4
FINETUNE_UNFREEZE_FROM = "layer3"  # unfreeze layer3 + layer4 + fc (resnet18 layer name;
                                    # for efficientnet_b0 / convnext_tiny, train.py passes
                                    # each architecture's own default — see --unfreeze-from)
